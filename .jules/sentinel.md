...
## 2024-05-29 - Cross-Site Scripting (XSS) in AI Chatbot Widget
**Vulnerability:** The AI chatbot widget in `resources/views/layouts/app.blade.php` rendered user and bot messages using AlpineJS's `x-html` directive. The markdown was parsed via `marked.parse()` but was not sanitized, allowing for arbitrary HTML and script execution (XSS) if malicious content was returned or injected.
**Learning:** AlpineJS's `x-html` directive functions similarly to `innerHTML`, rendering whatever HTML is provided directly into the DOM. When rendering dynamic content, especially markdown generated by AI or untrusted input, it must always be sanitized to prevent malicious script execution.
**Prevention:** Always wrap content passed to `x-html` in a sanitizer like `DOMPurify.sanitize()` (e.g., `DOMPurify.sanitize(marked.parse(msg.text))`). This ensures that only safe HTML tags and attributes are rendered, mitigating XSS risks.
