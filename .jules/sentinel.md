...
## 2026-06-19 - Missing Rate Limiting on External API Calls in Livewire
**Vulnerability:** The `generateSeoAndTags` method in the `ManagePosts` Livewire component lacked rate limiting while communicating with an external AI API, exposing the application to resource exhaustion and Denial of Wallet (DoW) attacks.
**Learning:** Livewire methods that invoke third-party APIs can be rapidly triggered. In addition, when fixing this, using `Auth::id()` without importing the Facade caused a fatal error, whereas using the global helper `auth()->id()` is safer.
**Prevention:** Always implement explicit rate limiting using Laravel's `RateLimiter` facade on Livewire methods that trigger resource-intensive tasks or external API calls. Prefer using Laravel's global helper functions (e.g., `auth()->id()`) over Facades (e.g., `Auth::id()`) to reduce the risk of runtime errors caused by missing class imports.
