...
## 2026-07-04 - Prevent XSS via Unsanitized LLM Output
**Vulnerability:** Found an instance in `AiCoverLetter.php` where output from an external LLM was rendered directly without sanitization, posing a risk of XSS or malicious payload injection if the LLM hallucinates or is poisoned via prompt injection.
**Learning:** External AI models should be treated as untrusted data sources. Even if system prompts enforce formatting, the output can still include dangerous scripts or HTML smuggled inside markdown wrappers.
**Prevention:** Always wrap text generated by AI models with a rigorous output sanitizer (e.g., `AiSecurityService->sanitizeOutput()`) before saving it to the database or rendering it directly in the frontend component state.
