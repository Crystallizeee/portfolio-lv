...
## 2026-07-12 - [Missing Rate Limiting on Image Uploads]
**Vulnerability:** The `upload` method in `PostAttachmentController` lacked rate limiting, allowing an attacker to repeatedly upload files, potentially leading to storage exhaustion or Denial of Service (DoS) attacks.
**Learning:** Endpoints handling file uploads are susceptible to resource exhaustion attacks even when they require authentication. The memory, disk space, and CPU used in validation and storage can be overwhelmed.
**Prevention:** Always implement explicit rate limiting (e.g., using Laravel's `RateLimiter` facade) on file upload endpoints, capping the number of uploads a user can make within a given time period.
