...
## 2024-05-24 - Screen Reader Compatibility for Decorative Icons
**Learning:** Decorative icons (like Lucide `<i>` tags or raw `<svg>` tags) adjacent to descriptive text in interactive elements (e.g. contact form links, project links) cause screen readers to stumble or redundantly announce content if they lack proper ARIA attributes.
**Action:** Always apply `aria-hidden="true"` to decorative icons (such as `<i data-lucide="..."></i>` or `<svg>`) when they are used alongside descriptive text within interactive elements.
