...
## 2026-10-24 - ARIA Roles and Keyboard Navigation for Custom Toggles
**Learning:** Found that custom toggle switches implemented using `div` elements (e.g., in the CV Generator) were inaccessible. They lacked `role="switch"`, `aria-checked` states, `tabindex="0"` for keyboard focus, and `keydown` event handlers (`enter` and `space`) to toggle the state via keyboard, as well as visible focus indicators (`focus-visible:ring`).
**Action:** When creating or updating custom toggle switches using `div` or generic elements, always add `role="switch"`, `aria-checked`, `tabindex="0"`, keyboard event handlers (like `wire:keydown.enter` and `wire:keydown.space.prevent`), and explicit `focus-visible` ring utilities to ensure they are fully accessible to screen readers and keyboard users.
