...
## 2024-03-21 - External Links Accessibility & Security
**Learning:** External links with `target="_blank"` lack `rel="noopener noreferrer"` (security/performance) and `sr-only` text warning users of the behavior (accessibility). Additionally, inline decorative SVGs and Lucide icons within link cards can cause redundant/confusing announcements by screen readers if they lack `aria-hidden="true"`.
**Action:** Always add `rel="noopener noreferrer"` and `<span class="sr-only">(opens in a new tab)</span>` to external links. Add `aria-hidden="true"` to decorative icons within link containers.
