...
## 2026-10-24 - Focus States and External Links in Certificates Section
**Learning:** Found that the interactive elements in the certificates section (such as the "Read more..." toggle button and the "Verify Credential" external link) lacked visible focus states for keyboard navigation. Additionally, the external link lacked screen reader text indicating it opens in a new tab, and the decorative Lucide icons were not hidden from screen readers, causing unnecessary noise.
**Action:** Always add explicit `focus-visible` utility classes (e.g., `focus:outline-none focus-visible:ring-2`) to interactive elements like buttons and links to ensure keyboard accessibility. For external links, use `<span class="sr-only">(opens in a new tab)</span>`, and ensure decorative icons use `aria-hidden="true"`.
