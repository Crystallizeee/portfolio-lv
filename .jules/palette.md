...
## 2024-08-01 - Enhance Security and Accessibility of External Links
**Learning:** When adding external links with `target="_blank"`, it is critical for both security and performance to include `rel="noopener noreferrer"`. Furthermore, to enhance screen reader accessibility, adding visually hidden text like `<span class="sr-only">(opens in a new tab)</span>` informs users of the new window behavior, and applying `aria-hidden="true"` to decorative icons (like SVGs or Lucide icons) prevents screen readers from redundantly announcing them.
**Action:** Always ensure `target="_blank"` links are paired with `rel="noopener noreferrer"`, visually hidden contextual text, and hidden decorative icons.
