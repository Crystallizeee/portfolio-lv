<div>
    ...
    {{-- Quick Add Buttons --}}
    @php
        // ⚡ Bolt Optimization: Instantiate the collection once to avoid redundant object
        // allocations and loop iterations when accessing $profiles multiple times in the view.
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button wire:click="toggleVisibility({{ $profile['id'] }})"
                                ...
                                title="{{ $profile['is_visible'] ? 'Hide' : 'Show' }}" aria-label="{{ $profile['is_visible'] ? 'Hide' : 'Show' }}">
                            <i data-lucide="{{ $profile['is_visible'] ? 'eye' : 'eye-off' }}" aria-hidden="true" class="w-4 h-4"></i>
                        </button>
