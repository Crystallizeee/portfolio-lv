    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex flex-col items-end">
            <button
                ...
                wire:loading.class="opacity-50 cursor-wait disabled:opacity-50 disabled:cursor-not-allowed"
                wire:target="generatePdf"
            >
                <i wire:loading.remove wire:target="generatePdf" data-lucide="download" class="w-4 h-4"></i>
                <i wire:loading wire:target="generatePdf" data-lucide="loader-2" class="w-4 h-4 animate-spin"></i>
                <span class="font-medium" wire:loading.remove wire:target="generatePdf">Generate PDF</span>
                <span class="font-medium" wire:loading wire:target="generatePdf">Generating...</span>
            </button>
            @error('generatePdf') <span class="text-red-400 text-xs mt-2 block">{{ $message }}</span> @enderror
        </div>
    </div>
                    ...
                    @foreach($educations as $index => $education)
                        <div class="p-5 bg-slate-800/40 rounded-xl border border-slate-700/50 relative group transition-all hover:border-slate-600 hover:bg-slate-800/60">
                            <div class="flex justify-between items-start mb-4">
                                ...
                                <button wire:click="removeEducation({{ $index }})" class="text-slate-500 hover:text-red-400 transition-colors p-2 hover:bg-red-500/10 rounded-lg" title="Remove Education #{{ $index + 1 }}" aria-label="Remove Education #{{ $index + 1 }}">
                ...
                <div class="space-y-4">
                    @foreach($certifications as $index => $cert)
                        <div class="p-4 bg-slate-800/50 rounded-lg border border-slate-700 relative group transition-all hover:border-slate-600">
                            <button wire:click="removeCertification({{ $index }})" class="absolute top-2 right-2 text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all p-1" title="Remove Certification #{{ $index + 1 }}" aria-label="Remove Certification #{{ $index + 1 }}">
                                <i data-lucide="trash-2" class="w-4 h-4"></i>
                            </button>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
            ...
            <div class="glass-card p-6 rounded-xl border border-slate-700/50">
                ...
                <div class="space-y-4">
                    @foreach($manualExperiences as $index => $exp)
                        <div class="p-4 bg-slate-800/50 rounded-lg border border-slate-700 relative group transition-all hover:border-slate-600">
                            <button wire:click="removeManualExperience({{ $index }})" class="absolute top-2 right-2 text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all p-1" title="Remove Work Experience #{{ $index + 1 }}" aria-label="Remove Work Experience #{{ $index + 1 }}">
                                <i data-lucide="trash-2" class="w-4 h-4"></i>
                            </button>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        @foreach($manualSkills as $index => $skill)
                            <div class="relative group">
                                ...
                                <button wire:click="removeManualSkill({{ $index }})" class="absolute top-1/2 -translate-y-1/2 right-2 text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all" title="Remove Skill #{{ $index + 1 }}" aria-label="Remove Skill #{{ $index + 1 }}">
            ...
            <div class="glass-card p-6 rounded-xl border border-slate-700/50">
                ...
                <div class="space-y-4">
                    @foreach($manualLanguages as $index => $lang)
                        <div class="p-4 bg-slate-800/50 rounded-lg border border-slate-700 relative group transition-all hover:border-slate-600">
                            <button wire:click="removeManualLanguage({{ $index }})" class="absolute top-2 right-2 text-slate-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all p-1" title="Remove Language #{{ $index + 1 }}" aria-label="Remove Language #{{ $index + 1 }}">
                                <i data-lucide="trash-2" class="w-4 h-4"></i>
                            </button>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
            ...
            <div class="glass-card p-6 rounded-xl border border-slate-700/50 sticky top-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="pt-4 mt-2 border-t border-slate-700/50">
                        ...
                        >
                            <span wire:loading.remove wire:target="generatePdf" class="flex items-center space-x-2">
                                <i data-lucide="download" class="w-5 h-5"></i>
                                <span>Generate CV</span>
                            </span>
                            <span wire:loading wire:target="generatePdf" class="flex items-center space-x-2">
                                <i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i>
                                <span>Generating...</span>
                            </span>
                        </button>
                        @error('generatePdf') <span class="text-red-400 text-xs mt-2 block text-center">{{ $message }}</span> @enderror
                    </div>
                </div>
            </div>
        </div>
