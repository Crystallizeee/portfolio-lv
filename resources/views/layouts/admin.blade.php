...
<body class="bg-[var(--color-cyber-dark)] text-slate-300 antialiased min-h-screen">
    <a href="#main-content" class="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[200] focus:px-4 focus:py-2 focus:bg-cyan-500 focus:text-white focus:rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900">Skip to content</a>
            ...
            <nav class="flex-1 p-4 space-y-1.5 overflow-y-auto custom-scrollbar overflow-x-hidden">
                ...
                @foreach($navItems as $item)
                    <a href="{{ route($item['route']) }}" 
                       class="group flex items-center px-4 py-2.5 rounded-xl transition-all duration-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900 {{ request()->routeIs($item['route']) ? 'bg-cyan-500/15 text-cyan-400 shadow-[inset_0_0_12px_rgba(34,211,238,0.1)]' : 'text-slate-400 hover:bg-white/5 hover:text-slate-200' }}"
        ...
        <div class="flex flex-col flex-1 min-w-0 overflow-hidden relative transition-all duration-500">
            ...
            <header class="h-16 mx-4 mt-4 glass-card flex items-center justify-between px-6 z-30 transition-all duration-500">
                <div class="flex items-center space-x-4">
                    <!-- Mobile Burger -->
                    <button type="button" @click="sidebarOpen = true" class="md:hidden p-2 text-slate-400 hover:text-white transition-all bg-white/5 rounded-lg active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400" aria-label="Open menu" :aria-expanded="sidebarOpen.toString()" aria-controls="admin-sidebar">
                    ...
                    <div class="flex space-x-2 items-center">
                        ...
                        <button type="button" @click="toggleCollapse()" class="p-1.5 text-slate-500 hover:text-cyan-400 transition-colors rounded-lg bg-white/5 md:hidden focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400" aria-label="Toggle sidebar width">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ url('/') }}" target="_blank" 
                       class="hidden sm:flex items-center space-x-2 px-4 py-2 rounded-lg bg-white/5 border border-white/5 text-slate-300 hover:text-cyan-400 hover:bg-white/10 hover:border-cyan-500/30 transition-all duration-300 text-sm font-medium group focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-400">
                    ...
                    <form action="{{ route('admin.clear-cache') }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to clear all system caches?');">
                        @csrf
                        <button type="submit"
                           class="hidden sm:flex items-center space-x-2 px-4 py-2 rounded-lg bg-white/5 border border-white/5 text-slate-300 hover:text-amber-400 hover:bg-white/10 hover:border-amber-500/30 transition-all duration-300 text-sm font-medium group focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400">
