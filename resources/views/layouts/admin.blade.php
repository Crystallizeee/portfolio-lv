        ...
        <div class="flex flex-col flex-1 min-w-0 overflow-hidden relative transition-all duration-500">
            ...
            <header class="h-16 mx-4 mt-4 glass-card flex items-center justify-between px-6 z-30 transition-all duration-500">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <form method="POST" action="{{ route('admin.clear-cache') }}" class="hidden sm:flex m-0" onsubmit="return confirm('Are you sure you want to clear all system caches?');">
                        @csrf
                        <button type="submit"
                                class="flex items-center space-x-2 px-4 py-2 rounded-lg bg-white/5 border border-white/5 text-slate-300 hover:text-amber-400 hover:bg-white/10 hover:border-amber-500/30 transition-all duration-300 text-sm font-medium group cursor-pointer">
